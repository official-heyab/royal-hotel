                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">List of all dinner table reservations</h6>
                        </div>
                        <div class="card-body">
                            <div class="table">
                                <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        @foreach($dinnerTableReservations as $dtr)
                                        @if($dtr->reservation_id!='')
                                            <tr>
                                                <td>{{ $dtr->reservation_name }}</td>
                                                <td>{{ $dtr->arrival}}</td>
                                                <td>{{ $dtr->remark }}</td>
                                                <td>{{ $dtr->phone_number }}</td>
                                                <td>
                                                    @if ($dtr->status)
                                                    <a href="{{ route('reserve.service.callback', ['sID' => $dtr->service_id, 'id' => $dtr->reservation_id]) }}" class="btn btn-primary btn-icon-split">
                                                        <span class="icon text-white-50"><i class="fas fa-info-circle"></i></span>
                                                        <span class="text">Call Back</span>
                                                    </a>
                                                    @else
                                                    <a href="{{ route('reserve.service.done', ['sID' => $dtr->service_id, 'id' => $dtr->reservation_id]) }}" class="btn btn-success btn-icon-split">
